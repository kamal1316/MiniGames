...
def create_grid(locked_pos):
    grid = [[(0, 0, 0) for _ in range(n_cols)] for _ in range(n_rows)]
...
def valid_space(shape, grid):
    accepted_pos = [[(j, i) for j in range(n_cols) if grid[i][j] == (0, 0, 0)] for i in range(n_rows)]
...
def main_menu(window):
    ...
    while run:
        window.fill((0, 0, 0))
        draw_text_middle(window, "Press any key to begin", 50, (255, 255, 255))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
